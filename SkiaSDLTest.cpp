    ...
    const uint32_t _Val) noexcept
{
	...
	while (_Ptr != _Last && *_Ptr != _Val) { ++_Ptr; }
	return _Ptr;
}
    ...
    uint8_t flags)
{
	...
	for (int i = 0; i < numArgs; ++i)
	{
		...
		switch (argTypes[i])
		{
			...
			case TRACE_VALUE_TYPE_COPY_STRING: {
				...
				if (newLineAt > 0) { truncAt = newLineAt; }
	...
	if (open)
	{
		...
		SkDebugf("[% 2d]%s <%s> %s%s #%d {\n", fIndent.size(), fIndent.c_str(), category,
...
static void
nsvgRender(SkCanvas* canvas, NSVGimage* image, float bounds[4])
{
	static SkPaint::Cap nsvgCap[] = {
	    SkPaint::kButt_Cap, SkPaint::kRound_Cap, SkPaint::kSquare_Cap};
	static SkPaint::Join nsvgJoin[] = {
	    SkPaint::kMiter_Join, SkPaint::kRound_Join, SkPaint::kBevel_Join};

	float w = bounds[2] - bounds[0];
	float h = bounds[3] - bounds[1];
	float s = w / image->width < h / image->height ? w / image->width : h / image->height;
	canvas->save();
		...
		if (shape->paths)
		{
			...
			for (NSVGpath* path = shape->paths; path != NULL; path = path->next)
			{
				...
				for (int i = 1; i < path->npts - 1; i += 3)
				{
					...
					if (fabsf((p[2] - p[0]) * (p[5] - p[1]) -
					          (p[4] - p[0]) * (p[3] - p[1])) < 0.001f)
			...
			if (shape->fill.type != NSVG_PAINT_NONE)
			{
				...
				else if (shape->fill.type == NSVG_PAINT_LINEAR_GRADIENT)
				{
					...
					SkPoint pts[] = {
					    {x0 + s0->offset * dx, y0 + s0->offset * dy},
					    {x0 + s1->offset * dx, y0 + s1->offset * dy}};
					auto lg = SkGradientShader::MakeLinear(
					    pts, colors, positions, 2, SkTileMode::kMirror);
					paint.setShader(lg);
				}
...
static void
svgTest(SkCanvas* canvas, const char* filename, int fbWidth, int fbHeight)
{
	...
	float bounds[4] = {0, 0, static_cast<float>(fbWidth), static_cast<float>(fbHeight)};
...
static void
handle_events(ApplicationState* state, SkCanvas* canvas)
{
	SDL_Event event;
	while (SDL_PollEvent(&event))
	{
		switch (event.type)
		{
			...
			case SDL_KEYDOWN: {
				...
				if (key == SDLK_ESCAPE) { state->fQuit = true; }
				break;
			}
...
create_star()
{
	...
	for (int i = 1; i < kNumPoints; ++i) { rot.mapPoints(points + i, points + i - 1, 1); }
...
main(int argc, char** argv)
{
	...
	{
		if (strcmp(argv[argi], "--sw") == 0 && ++argi < argc) swRender = atoi(argv[argi]);
		...
		if (strcmp(argv[argi], "--svg") == 0 && ++argi < argc) svgFile = argv[argi];
	}
	...
	SDL_Window* sdlWindow = SDL_CreateWindow(
	    "SDL Window", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, dm.w, dm.h,
	    windowFlags);
	...
	if (swRender == 0)
	{
		...
		else
		{
			...
			if (SDL_GL_CONTEXT_PROFILE_ES == contextType) { info.fFormat = GR_GL_BGRA8; }
			else
			{
		...
		target.reset(
		    new GrBackendRenderTarget(dw, dh, kMsaaSampleCount, kStencilBits, info));
	...
	{ // swRender > 0
		...
		auto imageInfo =
		    SkImageInfo::Make(dw, dh, kBGRA_8888_SkColorType, kUnpremul_SkAlphaType);
		skSurface =
		    SkSurface::MakeRasterDirect(imageInfo, sdlSurface->pixels, sdlSurface->pitch);
	}
	...
	if (doTrace) SkEventTracer::SetInstance(new SkDebugfTracer());
 ...
 */
	...
	while (!state.fQuit)
	{
		...
		//  //canvas->scale((float)dw/dm.w, (float)dh/dm.h);
		//}
		if (swRender != 0) SDL_LockSurface(sdlSurface);
		...
		if (swRender == 0) { SDL_GL_SwapWindow(sdlWindow); }
		else
		{
